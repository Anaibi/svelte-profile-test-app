var app = (function () {
	...
	function setStars(node) {
		var rating = this.get().profile.rating,
				addedStars = 0; rating = 3.5;
	...
	function create_main_fragment$19(component, ctx) {
		var a, img, img_src_value, img_alt_value, a_href_value, text_1, p, strong, text_2_value = ctx.post.user.name, text_2, text_3, span, span_1, text_4, text_5_value = ctx.post.date, text_5, text_6, span_2, text_8, button, text_10, div, text_11, div_1, div_2, raw_value = ctx.post.content.text, text_13, button_1;

		function select_block_type(ctx) {
			if (ctx.post.content.img) return create_if_block$2;
		...
		return {
			c: function create() {
				...
				text_13 = createText("\n    ");
				button_1 = createElement("button");
				button_1.textContent = "Hire";
				img.src = img_src_value = ctx.post.user.img;
				img.alt = img_alt_value = ctx.post.user.name;
				a.href = a_href_value = ctx.post.user.link;
				span_1.className = "icon";
				span.className = "note";
				span_2.className = "right note italic";
			...
			p: function update(changed, ctx) {
				if ((changed.post) && img_src_value !== (img_src_value = ctx.post.user.img)) {
					img.src = img_src_value;
				}

				if ((changed.post) && img_alt_value !== (img_alt_value = ctx.post.user.name)) {
					img.alt = img_alt_value;
				}

				if ((changed.post) && a_href_value !== (a_href_value = ctx.post.user.link)) {
					a.href = a_href_value;
				}

				if ((changed.post) && text_2_value !== (text_2_value = ctx.post.user.name)) {
					text_2.data = text_2_value;
				}

				if ((changed.post) && text_5_value !== (text_5_value = ctx.post.date)) {
					text_5.data = text_5_value;
				}
	...
	// (13:10) {#each post.recommendations.users as user}
	...
	// (17:12) {#if post.recommendations.rating}
	...
	// (20:12) {#if post.recommendations.total > 0}
	...
	// (29:6) {#each post.recommendations.users as user}
	...
	// (33:8) {#if post.recommendations.rating}
	...
	// (36:8) {#if post.recommendations.total > 0}
	...
	function create_main_fragment$20(component, ctx) {
		var a, img, img_src_value, img_alt_value, a_href_value, text_1, div, div_1, div_2, a_1, img_1, img_1_src_value, img_1_alt_value, a_1_href_value, text_3, div_3, p, text_4_value = ctx.post.user.name, text_4, text_5, span, text_8, div_4, span_1, text_9_value = ctx.post.review.rating, text_9, text_10, span_2, text_14, p_1, text_15, raw_value = ctx.post.review.text, raw_before, raw_after, text_16, text_17, p_2, strong, text_18_value = ctx.post.review.user.name, text_18;
		...
		return {
			c: function create() {
				...
				img.src = img_src_value = ctx.post.review.user.img;
				img.alt = img_alt_value = ctx.post.review.user.name;
				a.className = "tile-round";
				a.href = a_href_value = ctx.post.review.user.link;
				img_1.src = img_1_src_value = ctx.post.user.img;
				img_1.alt = img_1_alt_value = ctx.post.user.name;
				a_1.className = "tile-round";
				a_1.href = a_1_href_value = ctx.post.user.link;
				span.className = "icon-stars";
			...
			m: function mount(target, anchor) {
				...
				appendNode(text_18, strong);
			},

			p: function update(changed, ctx) {
				if ((changed.post) && img_src_value !== (img_src_value = ctx.post.review.user.img)) {
					img.src = img_src_value;
				}

				if ((changed.post) && img_alt_value !== (img_alt_value = ctx.post.review.user.name)) {
					img.alt = img_alt_value;
				}

				if ((changed.post) && a_href_value !== (a_href_value = ctx.post.review.user.link)) {
					a.href = a_href_value;
				}

				if ((changed.post) && img_1_src_value !== (img_1_src_value = ctx.post.user.img)) {
					img_1.src = img_1_src_value;
				}

				if ((changed.post) && img_1_alt_value !== (img_1_alt_value = ctx.post.user.name)) {
					img_1.alt = img_1_alt_value;
				}

				if ((changed.post) && a_1_href_value !== (a_1_href_value = ctx.post.user.link)) {
					a_1.href = a_1_href_value;
				}

				if ((changed.post) && text_4_value !== (text_4_value = ctx.post.user.name)) {
					text_4.data = text_4_value;
				}

				if ((changed.post) && text_9_value !== (text_9_value = ctx.post.review.rating)) {
					text_9.data = text_9_value;
				}

				if ((changed.post) && raw_value !== (raw_value = ctx.post.review.text)) {
					detachBetween(raw_before, raw_after);
					raw_before.insertAdjacentHTML("afterend", raw_value);
				}

				if ((changed.post) && text_18_value !== (text_18_value = ctx.post.review.user.name)) {
	...
	function create_main_fragment$21(component, ctx) {
		var a, img, img_src_value, img_alt_value, a_href_value, text_1, p, strong, text_2_value = ctx.post.user.name, text_2, text_3, span, span_1, text_4, text_5_value = ctx.post.date, text_5, text_7, div, raw_value = ctx.post.content.text;
	...
	function create_each_block$3(component, ctx) {
		...
		function select_block_type(ctx) {
			if (ctx.post.isSponsored) return create_if_block$3;
			if (ctx.post.isReview) return create_if_block_1$2;
	...
	function create_if_block_1$2(component, ctx) {
		...
		var reviewpost_initial_data = { post: ctx.post };
	...
	function create_main_fragment$25(component, ctx) {
		...
		function switch_props(ctx) {
			var switch_instance_initial_data = { profile: ctx.profile };
			return {
				root: component.root,
		...
		return {
			c: function create() {
				...
				div = createElement("div");
				if (switch_instance) switch_instance._fragment.c();
				ul.className = "tab-nav svelte-1x2evwv";
	...
	function create_each_block$4(component, ctx) {
		...
		function select_block_type(ctx) {
			if (ctx.item.icon.svg) return create_if_block$4;
		...
		return {
			c: function create() {
				...
				span = createElement("span");
				text_1 = createText(text_1_value);
				span.className = "text svelte-1x2evwv";

				button._svelte = { component, ctx };

				addListener(button, "click", click_handler);
				button.className = button_class_value = "menu-item " + (ctx.item.active ? 'active' : '') + " svelte-1x2evwv";
				li.className = "svelte-1x2evwv";
			},
			...
			p: function update(changed, ctx) {
				...
				if ((changed.menu) && button_class_value !== (button_class_value = "menu-item " + (ctx.item.active ? 'active' : '') + " svelte-1x2evwv")) {
	...
	function create_if_block$4(component, ctx) {
		...
		return {
			c: function create() {
				span = createElement("span");
				iconssvg._fragment.c();
				span.className = "icon svelte-1x2evwv";
			},
	...
	/* src/Users.html generated by Svelte v2.4.4 */
	...
	function create_main_fragment$28(component, ctx) {
		...
		var contentprofile_initial_data = { menu: ctx.profileMenu, profile: ctx.profile };
		...
		return {
			c: function create() {
				...
				link.rel = "stylesheet";
				header.className = "row vCentered svelte-7xyf3";
				div_4.className = "content svelte-7xyf3";
				div_3.className = "row";
				main.className = "svelte-7xyf3";
				div_2.className = "row";
				div_1.className = "scroll";
				div.className = "page sidenav svelte-7xyf3";
			},
